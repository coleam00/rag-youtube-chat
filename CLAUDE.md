...
3. **Retrieval** uses hybrid Reciprocal Rank Fusion (RRF) combining Postgres tsvector keyword search with pgvector cosine similarity. RRF over-fetches top-k × 4 candidates, then a cross-encoder reranker (`cross-encoder/ms-marco-MiniLM-L-6-v2`) re-scores and returns top-k. Per-video cap and small-to-big neighbor expansion follow. Do not introduce a different reranker model without an explicit issue authorizing it — that's an architectural change requiring human approval.
